...
async changeInsertionModeSetting(mode: InsertionMode) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("change_insertion_mode_setting", { mode }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
async setSelectedMicrophone(deviceName: string, deviceId: string | null) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("set_selected_microphone", { deviceName, deviceId }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
async getHistoryEntries(cursor: number | null, limit: number | null, search: string | null, startTimestamp: number | null, endTimestampExclusive: number | null, modelFilter: string | null, outcomeFilter: string | null, cleanupFilter: string | null) : Promise<Result<PaginatedHistory, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("get_history_entries", { cursor, limit, search, startTimestamp, endTimestampExclusive, modelFilter, outcomeFilter, cleanupFilter }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
async retryHistoryEntryCleanup(id: number) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("retry_history_entry_cleanup", { id }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
 ...
 */
selected_channel?: number | null; clamshell_microphone?: string | null; 
/**
 ...
 */
pause_media_while_recording?: boolean; append_trailing_space?: boolean; app_language?: string; theme?: Theme; experimental_enabled?: boolean; lazy_stream_close?: boolean; keyboard_implementation?: KeyboardImplementation; show_tray_icon?: boolean; paste_delay_ms?: number; paste_delay_after_ms?: number; 
/**
...
export type AudioDevice = { index: string; stable_id: string | null; name: string; is_default: boolean }
 ...
 */
export type InsertionMode = "at_stop" | "preview_only" | "live_committed_experimental"
